...
from django.dispatch import receiver
from assistencia.models import PecaOrdemServico
...
@receiver(pre_save, sender=PecaOrdemServico)
def salvar_quantidade_antiga_peca(instance, **kwargs):
    if instance.pk:
        try:
            peca_os_antiga = PecaOrdemServico.objects.get(pk=instance.pk)
            instance._quantidade_antiga = peca_os_antiga.quantidade
            instance._peca_os_antiga = peca_os_antiga
        except PecaOrdemServico.DoesNotExist:
            instance._quantidade_antiga = 0
            instance._peca_os_antiga = None
    else:
        instance._quantidade_antiga = 0
        instance._peca_os_antiga = None
